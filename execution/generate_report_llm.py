...
[디자인 요구사항]
- 보고서 양식은 무조건 HTML 파일 형태로 작성되어야 하며, `<meta name="viewport" content="width=device-width, initial-scale=1.0">` 태그를 반드시 포함하세요.
- 모바일 기기(스마트폰)에서도 완벽하게 읽히도록 **반응형 웹 디자인(Responsive Design)**을 적용하세요. (미디어 쿼리 사용, 가로 스크롤 방지, 텍스트 크기 조정 등)
- 손가락으로 터치하기 쉽도록 버튼과 링크의 여백(Padding)을 넉넉하게 주고, 카드 UI가 모바일 화면 너비에 꽉 차면서도 보기 좋게 자동 정렬(flex-wrap 등)되도록 만드세요.
- 내부에 `<style>` 태그를 사용하여 그라데이션 배경, 그림자 효과를 넣은 모던하고 예쁜 프리미엄 카드 UI를 적용해 주세요.
- 모든 출처 표기는 기사 제목이나 `[출처 확인]` 텍스트를 클릭 가능한 HTML `<a>` 태그(예: `<a href="https://..." target="_blank">기사 원문</a>`)로 만들고 눈에 잘 띄게 꾸며주세요.
- 마크다운 문법 대신 순수 HTML/CSS 태그만 반환하세요. 출력 맨 앞과 맨 뒤에 ```html 같은 코드블록 기호를 넣지 마세요.
"""
